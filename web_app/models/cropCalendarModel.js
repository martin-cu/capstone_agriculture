...
exports.getCropCalendars = function(query, next) {
	...
	var sql = "select * from ( SELECT max(farm_area) as farm_area, max(calendar_id) as calendar_id, max(farm_id) as farm_id, max(land_prep_date) as land_prep_date, max(sowing_date) as sowing_date, max(harvest_date) as harvest_date, max(planting_method) as planting_method, max(seed_planted) as seed_planted, max(status) as status, max(seed_rate) as seed_rate, max(harvest_yield) as harvest_yield, max(crop_plan) as crop_plan, max(method) as method, max(farm_name) as farm_name, max(land_type) as land_type, max(seed_name) as seed_name, max(maturity_days) as maturity_days, max(sow_type) as sow_type, max(sow_date_completed) as sow_date_completed, max(lp_type) as lp_type, max(lp_date_completed) as lp_date_completed, CASE WHEN MAX(t.lp_type) IS NOT NULL THEN CASE WHEN MAX(sow_date_completed) IS NULL THEN 'Sowing' WHEN DATEDIFF(NOW(), MAX(sow_date_completed)) < maturity_days THEN 'Vegetation' WHEN DATEDIFF(NOW(), MAX(sow_date_completed)) >= maturity_days && DATEDIFF(NOW(), MAX(sow_date_completed)) < (maturity_days + 35) THEN 'Reproductive' WHEN DATEDIFF(NOW(), MAX(sow_date_completed)) >= (maturity_days + 35) && DATEDIFF(NOW(), MAX(sow_date_completed)) < (maturity_days + 65) THEN 'Ripening' WHEN DATEDIFF(NOW(), MAX(sow_date_completed)) >= (maturity_days + 65) THEN 'Harvesting' ELSE 'Sowing' END ELSE 'Land Preparation' END AS stage FROM (SELECT cct.*, ft.farm_area, ft.farm_name, ft.land_type, st.seed_name, st.maturity_days, NULL AS sow_type, NULL AS sow_date_completed, NULL AS lp_type, NULL AS lp_date_completed FROM crop_calendar_table cct JOIN farm_table ft ON cct.farm_id = ft.farm_id JOIN seed_table st ON cct.seed_planted = st.seed_id";
...
exports.getRequiredMaterials = function(query, next) {
	var sql1 = "select *, case when type = 'Fertilizer' then (select fertilizer_name from fertilizer_table where fertilizer_id = item_id) when type = 'Seed' then (select seed_name from seed_table where seed_id = item_id) when type = 'Pesticide' then (select pesticide_name from pesticide_table where pesticide_id = item_id) end as item_name, case when (current_amount - qty ) < 0 then abs(current_amount - qty ) else 'N/A' end as deficient_qty from ( select farm_id, type, qty, item_id, max(current_amount) as current_amount from ( select cct.farm_id, wort.type, sum(wort.qty) as qty, wort.item_id, null as current_amount from crop_calendar_table cct join work_order_table wot on cct.calendar_id = wot.crop_calendar_id join wo_resources_table wort using(work_order_id) where ";
	...
	var sql3 = " and isActive = 1 ) as t1 group by farm_id, t1.type, t1.item_id ) as t2 join farm_table ft using(farm_id) where t2.qty is not null"
